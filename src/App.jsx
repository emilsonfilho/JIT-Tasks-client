...
function App() {
  ...
  const [completedTodayAmount, setCompletedTodayAmount] = useState(0);
  ...
  const fetchAllData = () => {
    ...
    fetch("http://localhost:3000/tasks/metrics")
      ...
      .then((data) => {
        ...
        setCompletedTodayAmount(data.done_today);
        setCompletedAmount(data.total_done);
      });
  ...
  const handleDeleteTask = async (task) => {
    const confirmDelete = window.confirm(
      "Essa é uma operação irreversível. Deseja continuar?",
    );
    if (confirmDelete) {
    ...
    <div className="flex bg-gray-200 text-slate-800 h-screen overflow-hidden">
      ...
      <main className="flex-1 flex flex-col p-8 overflow-hidden bg-gray-100 gap-6">
        <div className="shrink-0 mt-8">
          ...
          <p className="mt-2">
            Você tem {overdatedAmount} itens atrasados, {pendingAmount} tarefas
            pendentes e concluiu {completedTodayAmount} tarefas hoje.{" "}
          </p>
        </div>
        <div className="flex flex-1 min-h-0 gap-6">
          <div className="flex-1 flex flex-col gap-6 min-h-0">
            <ColumnHeader
              title="Náo Finalizadas"
              amount={pendingTasks.length}
            />
