...
export default function TaskModal(props) {
  ...
  const [dueDate, setDueDate] = useState(
    props.taskToEdit?.due_date ? props.taskToEdit?.due_date.split("T")[0] : "",
  );
  ...
  useEffect(() => {
    fetch("http://localhost:3000/priorities/")
      ...
      .then((data) => {
        setPriorities(data);
        if (props.taskToEdit) {
          const taskPriority = data.find(
            (priority) => priority.id === props.taskToEdit.priority_id,
          );
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const url = isEditing
        ? `http://localhost:3000/tasks/${props.taskToEdit.id}`
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <fieldset className="grid grid-cols-2">
          <div className="flex flex-col gap-6">
            ...
            {priorityError && (
              <span className="text-red-500 text-sm font-medium mt-[-1rem]">
        ...
        <footer className="flex justify-end">
          <div className="flex gap-2">
            <ModalButton
              style="text-slate-600 hover:bg-slate-200"
              onClick={props.onClose}
            >
              Cancelar
            </ModalButton>
            <ModalButton
              style="bg-gray-500 text-white rounded-lg hover:bg-gray-600"
              type="submit"
            >
              Salvar
            </ModalButton>
